...
\author{
 M. Landa\textsuperscript{1}, O. Pe\v{s}ek\textsuperscript{1}, J. Je\v{r}\'{a}bek\textsuperscript{2}, P. Kavka\textsuperscript{2}}
...
\abstract{ SMODERP2D is a runoff-soil erosion physically-based
  ...
  flow. Stream network routing is performed line-by-line in the~user
  predefined polyline layer.

  SMODERP is a long-term project driven by the Department of Landscape
  Water Conservation at the Czech Technical University in Prague. At
  the beginning, SMODERP has been developed as a surface runoff
  simulated by profile model (1D). Later the model has been redesigned
  using a spatially distributed method. This version is named
  ...
  geospatial data processing. Importantly, the SMODERP2D project is currently
  entering an open source world. Originally the model could be run
  ...
  and QGIS plugin significantly increases the~accessibility of the
  SMODERP2D model for research purposes and also for engineering
  practice.

  Middle scale distributed hydrological models often encounter with
