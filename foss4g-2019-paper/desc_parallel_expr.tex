...
based on certain algorithm where each sub-domain computation is loaded to single
...
the hydrological reality and it is shown in a simplified setup in
the Figure~\ref{fig:cpu-parallel}. In this example the Nu\v{c}ice experimental 
...
hyetographs as the only input. In the simplified setup shown in the
...
This approach may encounter in several limitations. The main one 
originates from the basin geometry. In case of narrow  basin a situation 
